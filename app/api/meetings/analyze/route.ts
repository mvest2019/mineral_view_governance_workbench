...
import { mongoSaveMeeting } from '@/lib/mongo_bridge';
...
export const POST = route(async (req: NextRequest) => {
  ...
  // Dual-write: MongoDB first (best-effort, non-fatal) — stores the meeting
  // metadata (embedded attendees) and the uploaded file reference. GitHub
  // markdown remains authoritative for the response.
  await mongoSaveMeeting({
    company,
    title,
    meetingDate,
    uploadedBy,
    ...
    attendees: attendees.map((a) => ({
      team_member_key: String(a['team_member_key'] || '').trim(),
      external_name: String(a['external_name'] || '').trim(),
    })),
  });
