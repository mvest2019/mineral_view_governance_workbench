#!/usr/bin/env node
// Provision GovernanceDB.questionAssignments. IDEMPOTENT, WRITE-FREE. Targets
// ONLY GovernanceDB. Mirrors questionAssignments.validator.ts + .indexes.ts.
// Usage: npm run db:provision:question-assignments
import { MongoClient } from 'mongodb';

const DEFAULT_DB_NAME = 'GovernanceDB';
const COLLECTION = 'questionAssignments';
const QUESTION_KIND = ['PRIORITY', 'REPO'];

const JSON_SCHEMA = {
  bsonType: 'object',
  required: ['companyKey', 'questionCode', 'questionKind', 'assigneeKey',
...
const INDEXES = [
  { key: { companyKey: 1, questionCode: 1 }, name: 'ux_questionAssignments_company_code', unique: true, partialFilterExpression: { isDeleted: false } },
  { key: { companyKey: 1, assigneeKey: 1 }, name: 'ix_questionAssignments_company_assignee' },
];
