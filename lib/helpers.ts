...
export async function generate_priority_questions_from_task(
  ...
  // Accept one employee (Task Tracker) or several (Meeting attendees). Each
  // generated question is stored under every listed member so it appears on the
  // Priority Questions page under each of their names.
  const rawKeys = Array.isArray(employee_key) ? employee_key : [employee_key];
  ...
  if (!fresh.length) {
    return { ok: true, skipped: false, created: [], count: 0, question_count: 0, member_keys, employee_display };
  }
...
export async function apply_generated_questions(company: string, payload: Dict): Promise<void> {
  ...
  recompute_question_payload_counts(payload);
}

// Recompute employee bucket counts and the team-pill counts for a questions
// payload after questions have been added or removed.
function recompute_question_payload_counts(payload: Dict): void {
...
// Priority Questions page policy: keep only questions that are AI-generated
// (future questions) or assigned to Riya Wankhade (the one existing set kept on
// request). All other pre-existing/demo questions are hidden from the page.
export function filter_priority_questions_for_page(payload: Dict): void {
  const keep = (q: Dict) => {
    const isGenerated = String(q['source'] || '') === 'claude' || String(q['qid'] || '').startsWith('Q-AI');
    const isRiya = String(q['assignee'] || '').trim().toLowerCase() === 'riya wankhade';
    return isGenerated || isRiya;
  };
  payload['org_wide'] = (payload['org_wide'] as Dict[]).filter(keep);
  const employees = payload['employees'] as Dict;
  for (const key of Object.keys(employees)) {
    const bucket = employees[key] as Dict;
    bucket['questions'] = (bucket['questions'] as Dict[]).filter(keep);
    if (!(bucket['questions'] as Dict[]).length) {
